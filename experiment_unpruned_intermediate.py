...
# Results for each bin (for comp and suff)
...
for model_name in LMs:
    ...
    if os.path.exists(path_unpruned_model):
        ...
        num = 200 # Not used, remove from function
        ...
        for seed in [0,1,2]:
            ...
            if not os.path.isfile("./results/unpruned/intermediate/seeds/"+ model_name+'_lime_res_3d_'+str(seed)+'.json'):
                path_res_lm_eval = "./results/unpruned/"+ model_name+'.json'
                if os.path.isfile("./results/unpruned/seeds/"+ model_name+'_fa_fa_'+str(seed)+'.json'):
                    ...
                    out = compute_comp_suff_i(model, tokenizer, path_res_lm_eval, path_fa, task_list=tl_faith, device=device)
                    tmp = "./results/unpruned/intermediate/seeds/"+"/".join(model_name.split("/")[:-1])
                    if not os.path.exists(tmp):
                        os.makedirs(tmp)
                    with open("./results/unpruned/intermediate/seeds/"+ model_name+'_lime_res_3d_'+str(seed)+'.json', 'w+') as f:
                        json.dump(out,f)
    
            if not os.path.isfile("./results/unpruned/intermediate/seeds/"+ model_name+'_ks_res_3d_'+str(seed)+'.json'):
                path_res_lm_eval = "./results/unpruned/"+ model_name+'.json'   
                if os.path.isfile("./results/unpruned/seeds/"+ model_name+'_fa_kernel_shap_'+str(seed)+'.json'):
                    ...
                    if not os.path.exists(tmp):
                        os.makedirs(tmp)
                    with open("./results/unpruned/intermediate/seeds/"+ model_name+'_ks_res_3d_'+str(seed)+'.json', 'w+') as f:
                        json.dump(out,f)
