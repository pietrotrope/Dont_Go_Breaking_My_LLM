...
for model_name in LMs:
    ...
    for num, i in enumerate(order_to_remove[:int(len(order_to_remove)*0.5)]):
        ...
        for seed in [0,1,2]:
            ...
            if not os.path.isfile("./results/pruned/seeds/"+ model_name+"_"+str(num)+'_fa_fa_'+str(seed)+'.json'):
                ...
                if not os.path.exists(tmp):
                    os.makedirs(tmp)
                with open("./results/pruned/seeds/"+ model_name+"_"+str(num)+'_fa_fa_'+str(seed)+'.json', 'w+') as f:
                    json.dump(out,f)
