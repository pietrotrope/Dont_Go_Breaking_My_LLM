    ...
    for num, i in enumerate(order_to_remove[:int(len(order_to_remove)*0.5)]):
        ...
        for seed in [0,1,2]:
            ...
            if sys.argv[2] == "1":
                ...
                if not os.path.isfile("./results/pruned/intermediate/seeds/"+ model_name+"_"+str(num)+'_lime_res_3d_'+str(seed)+'.json'):
                    ...
                    if os.path.isfile("./results/pruned/seeds/"+ model_name+"_"+str(num)+'_fa_fa_'+str(seed)+'.json'):
                        ...
                        tmp = "./results/pruned/intermediate/seeds/"+"/".join(model_name.split("/")[:-1])
                        if not os.path.exists(tmp):
                            os.makedirs(tmp)
                        with open("./results/pruned/intermediate/seeds/"+ model_name+"_"+str(num)+'_lime_res_3d_'+str(seed)+'.json', 'w+') as f:
                            json.dump(out,f)
            else:
                print("Kernel Shap")
                
                
                if not os.path.isfile("./results/pruned/intermediate/seeds/"+ model_name+"_"+str(num)+'_ks_res_3d_'+str(seed)+'.json'):
                    ...
                    if os.path.isfile("./results/pruned/seeds/"+ model_name+"_"+str(num)+'_fa_kernel_shap_'+str(seed)+'.json'):
                        ...
                        if not os.path.exists(tmp):
                            os.makedirs(tmp)
                        with open("./results/pruned/intermediate/seeds/"+ model_name+"_"+str(num)+'_ks_res_3d_'+str(seed)+'.json', 'w+') as f:
                            json.dump(out,f)
